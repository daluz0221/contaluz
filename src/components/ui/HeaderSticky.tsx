...
export const HeaderSticky = () => {
 const [isOpen, setIsOpen] = useState(false);
    ...
    useEffect(() => {
        const currentH = heroRef.current
        const observer = new IntersectionObserver(
        ([entry]) => {
            setScrolledPastHero(!entry.isIntersecting);
        },
        { threshold: 0 } // se dispara cuando cualquier parte deja de estar visible
        );

        if (currentH) observer.observe(currentH);

        return () => {
        if (currentH) observer.unobserve(currentH);
        };
    }, []);
    ...
    return (
        <>
            <header className={`
                    fixed top-0 flex justify-between items-center px-8 py-4 max-w-7xl mx-auto text-[var(--primary)] z-20 transition-all duration-500 ease-in-out ${scrolledPastHero ?  "opacity-100 block translate-y-0" : "hidden -translate-y-5 opacity-0 pointer-events-none"} bg-[var(--neutralFondo)]
                `}>
                {/* Logo */}
